...
def order_sucess(request, order_id):
    if order_id:
...
def add_cart(request, product_id):
    ...
    try:
        ...
        if cart_item.quantity < cart_item.product.stock:
            # checks if the amount being put into the cart is not more than the amount of stock available
...
def cart_detail(request, total=0, counter=0, cart_items = None):
    ...
    # stripe is in cents so we (* 100) to get it into euro
    description = 'EzKickz - New Order'
        ...
        try:
            ...
            try:
                ...
                for order_item in cart_items:
                    ...
                    # ^ the code above decreases the amount of stock when an item is bought
                    products.save()
                    order_item.delete()
                    print("Order created successfully")
                return redirect('order:order_success', order_details.id)
            except ObjectDoesNotExist:
                pass
        except stripe.error.CardError as e:
            return e
    return render(request, 'cart.html', {'cart_items':cart_items,
                                          'total':total, 'counter':counter, 'data_key':data_key,
